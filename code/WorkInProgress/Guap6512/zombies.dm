mob/living/carbon/var/zombie = 0
mob/living/carbon/var/becoming_zombie = 0
mob/living/carbon/var/zombieimmune = 0
mob/living/carbon/var/zombieleader = 0
...
/mob/living/carbon/human/proc/zombie_bit(var/mob/living/carbon/human/biter)
	...
	if(stat > 1)//dead: it takes time to reverse death, but there is no chance of failure
		...
		zombify()
		return
	if((istype(biten.wear_suit, /obj/item/clothing/suit/bio_suit) || istype(biten.wear_suit, /obj/item/clothing/suit/space)) || (istype(biten.head, /obj/item/clothing/head/bio_hood) || istype(biten.head, /obj/item/clothing/head/helmet/space)))
		if((istype(biten.head, /obj/item/clothing/head/bio_hood) || istype(biten.head, /obj/item/clothing/head/helmet/space)) && (istype(biten.wear_suit, /obj/item/clothing/suit/bio_suit) || istype(biten.wear_suit, /obj/item/clothing/suit/space)))
			if(prob(80))
