		...
		if(1.0)
			if (state < 2)
				...
				if (state < 3)
					...
					msg_admin_attack("ATTACK: [assailant.name] ([assailant.ckey]) grabbed the neck of [affecting.name] ([affecting.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[assailant.x];Y=[assailant.y];Z=[assailant.z]'>JMP</a>)",0)
					...
					if (state >= 3 && !killing)
						...
						if(do_after(assailant, 40))
							...
							msg_admin_attack("ATTACK: [assailant] ([assailant.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[assailant]'>JMP</A>) strangled [affecting] ([affecting.ckey]) (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[assailant.x];Y=[assailant.y];Z=[assailant.z]'>JMP</a>)", 0)
