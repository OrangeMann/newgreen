	...
	Bump(atom/A as mob|obj|turf|area)
		...
		if(firer && istype(A, /mob))
			...
			if(!def_zone)
				...
				if(istype(firer, /mob))
					...
					msg_admin_attack("ATTACK: [firer] ([firer.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[firer]'>JMP</A>) shot [M] ([M.ckey]) with [src].") //BS12 EDIT ALG
					log_attack("[firer] ([firer.ckey]) shot [M] ([M.ckey]) with [src.type]")
				else
					M.attack_log += "\[[time_stamp()]\] <b>UNKNOWN SUBJECT (No longer exists)</b> shot <b>[M]/[M.ckey]</b> with a <b>[src]</b>"
					msg_admin_attack("ATTACK: UNKNOWN (no longer exists) shot [M] ([M.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[M]'>JMP</A>) with [src].") //BS12 EDIT ALG
		...
		if(firer_obj && istype(A, /mob))
			...
			if (istype(A, /mob/living/carbon/human) || istype(A, /mob/living/carbon/monkey) || istype(A, /mob/living/silicon/robot) || istype(A, /mob/living/silicon/ai))
				msg_admin_attack("ATTACK: [firer_obj] from [firer_obj.x],[firer_obj.y],[firer_obj.z] shot [M]/[M.ckey] with [src]")
