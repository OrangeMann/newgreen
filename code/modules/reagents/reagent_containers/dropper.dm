	...
	afterattack(obj/target, mob/user , flag)
		...
		if(filled)
			...
			if(ismob(target))
				...
				msg_admin_attack("ATTACK: [user] ([user.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[user]'>JMP</A>) squirted [M.name] ([M.ckey]) with [src.name]. Reagents: [contained]", 0)
				log_attack("[user.name] ([user.ckey]) squirted [M.name] ([M.ckey]) with [src.name]. Reagents: [contained]")
