	...
	afterattack(obj/target, mob/user , flag)
		...
		if(filled)
			...
			if(ismob(target))
				...
				message_admins("ATTACK: [user] ([user.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[user]'>JMP</A>) squirted [M.name] ([M.ckey]) with [src.name]. Reagents: [contained]", 0)
