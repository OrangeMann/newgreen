...
/obj/item/weapon/reagent_containers/glass
	...
	afterattack(obj/target, mob/user , flag)
		...
		if(ismob(target) && target.reagents && reagents.total_volume)
			...
			message_admins("ATTACK: [user] ([user.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[user]'>JMP</A>) splashed [M.name] ([M.key]) with [src.name]. Reagents: [contained]", 0)
