	...
	afterattack(obj/target, mob/user , flag)
		...
		if(filled)
			...
			if(ismob(target))
				...
				msg_admin_attack("ATTACK: [user.name] ([user.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[user]'>JMP</A>) squirted [M.name] ([M.ckey]) with [src]. Reagents: [contained]", 0)
