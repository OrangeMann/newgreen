...
/obj/machinery/disposal
		...
		if(istype(G))	// handle grabbed mob
			if(ismob(G.affecting))
				...
				if(do_after(usr, 20))
					...
					msg_admin_attack("ATTACK: [user] ([user.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[user]'>JMP</A>) placed [GM.name] ([GM.ckey]) in a disposals unit at [get_area(src)]", 0)
	...
	MouseDrop_T(mob/target, mob/user)
		...
		if(target == user && !user.stat && !user.weakened && !user.stunned && !user.paralysis)	// if drop self, then climbed in
			...
			msg_admin_attack("ATTACK: [user] ([user.ckey])(<A HREF='?_src_=holder;adminplayerobservejump=\ref[user]'>JMP</A>) placed [target.name] ([target.ckey]) in a disposals unit at [get_area(src)]", 0)
